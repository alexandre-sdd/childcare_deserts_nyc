...
\noindent\textit{Candidate-site selection, under-\(5\) assignment, and spacing constraints.}
\[
\begin{alignedat}{2}
\sum_{s \in S} y_{ls} &\le 1, &\qquad
g_{ls} &\le U_s y_{ls},\\
\sum_{s \in S} y_{ls} &= 0 \quad \forall l \in L_{\text{blocked}},\\
...
The key policy insight is not only that the realistic model is more expensive, but that the structure of optimal intervention changes. Once expansion limits become binding and candidate-site spacing is enforced, the solution shifts away from incumbent expansion toward many more large new facilities. That change reflects the trade-off between implementation realism and cost efficiency in this planning setting.

Across both parts, the under-\(5\) requirement remains the dominant planning driver, often forcing excess total capacity in order to satisfy age-specific targets. In that sense, the model suggests that relaxing expansion caps or expanding the pool of feasible candidate sites could reduce modeled funding needs while still meeting the report's zipcode-level service requirements. Overall, the framework provides a structured basis for evaluating how implementation constraints change investment scale, facility mix, and the cost of eliminating coded childcare deserts.
