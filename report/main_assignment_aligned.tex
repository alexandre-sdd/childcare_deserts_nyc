...
The two-part structure is useful because the questions are different. Part 1 asks what the cost would be under optimistic implementation conditions. Part 2 asks how that answer changes when the city loses the ability to build anywhere, must respect site spacing, and can only expand incumbent facilities modestly.

\section{Parameter Estimation}

\subsection{Shared data cleaning}

The notebook pipeline uses five raw datasets supplied with the assignment: regulated childcare facilities, potential locations, employment rates, average individual income, and child population by zip code. The shared-cleaning stage standardizes zip codes to five-character strings, harmonizes column names, converts numeric fields, and preserves facilities even when geographic coordinates are missing. The cleaned shared outputs are then reused by both models.
...
\begin{tabularx}{\textwidth}{L{3.6cm}L{1.6cm}L{2.0cm}Y}
\toprule
\textbf{Shared output} & \textbf{Level} & \textbf{Shape} & \textbf{Used for} \\
\midrule
Clean facility table & facility & \(7{,}740 \times 20\) & Standardized facility master table with active flag, geo flag, under-\(5\) proxy, and quality flags \\
Facility geo-ready table & facility & \(7{,}740 \times 16\) & Compact facility subset used for realistic distance screening \\
Clean population table & zipcode & \(211 \times 22\) & Harmonized age bands plus derived child-population fields \\
Clean employment table & zipcode & \(179 \times 2\) & Zipcode employment covariate before imputation \\
Clean income table & zipcode & \(179 \times 2\) & Zipcode income covariate before imputation \\
Candidate geo-ready table & candidate & \(31{,}100 \times 5\) & Standardized candidate ID, zip code, coordinates, and geo flag \\
Master zipcode universe & zipcode & \(311 \times 12\) & Merge backbone with source flags, facility counts, baseline capacity, and candidate counts \\
\bottomrule
\end{tabularx}
...
The instructor-approved expansion-cap interpretation used in the notebook is
\[
...
The realistic parameter-estimation stage replaces the single benchmark cap with three cumulative-floor expansion blocks:
\[
...
\]
These correspond to the \(0\%\) to \(10\%\), \(10\%\) to \(15\%\), and \(15\%\) to \(20\%\) expansion bands, so total realistic expansion room at facility \(f\) is \(\lfloor 0.20\,n_f \rfloor\). The associated block-cost coefficients are
\[
...
\]
Under the original assignment Part 2 rule, the total expansion cost at facility \(f\) is
\[
C_f(x_f)=
\begin{cases}
...
\end{cases}
\]
This discontinuous regime-based total-cost rule creates upward jumps at the \(10\%\) and \(15\%\) thresholds because the higher regime coefficient applies to the full expansion level, not just to the incremental block. The processed inputs imply a citywide realistic expansion room of \(61{,}197\) slots. \Cref{fig:realistic-expansion-functions} illustrates the assignment-style example \(n_f=100\), where the breakpoints occur at \(10\), \(15\), and \(20\) added slots. The implemented solver later replaces this literal rule with an additive approximation for tractability. The shared benchmark demand table also carries the zero-child rule into both models, so zip codes with zero observed child population have zero total-slot requirement.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{realistic_expansion_functions.png}
\caption{Original assignment Part 2 total expansion cost rule for the assignment-style example \(n_f=100\). Dashed vertical segments mark the jumps at the \(10\%\) and \(15\%\) thresholds, where the higher regime coefficient applies to the entire expansion amount.}
...
Existing-existing close pairs (diagnostic only) & 7,644 \\
Facilities missing coordinates & 169 \\
\bottomrule
\end{tabular}
...
The benchmark model is defined over zip codes \(z \in Z\), active existing facilities \(f \in F\), and build sizes \(s \in S=\{\text{small},\text{medium},\text{large}\}\). The key decision variables are
\[
\begin{aligned}
...
\[
x_f \le U_f, \qquad u_f \le x_f, \qquad x_f \le (n_f-1)+U_f\delta_f,\qquad x_f \ge n_f\delta_f,
\]
\[
\mathrm{existing\_total}_z + \sum_{f \in F(z)}x_f + \sum_{s \in S}B_s y_{zs} \ge \mathrm{req\_total}_z,
\]
\[
\mathrm{existing\_under5}_z + \sum_{f \in F(z)}u_f + \sum_{s \in S}g_{zs} \ge \mathrm{req\_under5}_z,\qquad
g_{zs} \le U_s y_{zs}.
\]
Big-\(M\) linking constraints then force \(w_f = c_f^{(1)}x_f\) when \(\delta_f=0\) and \(w_f = c_f^{(2)}x_f\) when \(\delta_f=1\), so the whole expansion is priced under one regime.
...
Part 2 keeps the same demand targets but changes the intervention structure. Existing facilities may expand only within three capped bands, and new facilities can only be opened on observed candidate sites \(l \in L\). The key parameters are the three incumbent expansion caps \(\mathrm{cap1}_f,\mathrm{cap2}_f,\mathrm{cap3}_f\), the corresponding coefficients \(\mathrm{coef1}_f,\mathrm{coef2}_f,\mathrm{coef3}_f\), the blocked-site set \(L_{\text{blocked}}\), the within-zipcode conflict set \(\mathcal{C}\), and the build-menu parameters \(K_s,B_s,U_s\). The key decision variables are
\[
\begin{aligned}
...
\[
x_{1f} \le \mathrm{cap1}_f,\qquad x_{2f} \le \mathrm{cap2}_f,\qquad x_{3f} \le \mathrm{cap3}_f,\qquad
u_f \le x_{1f}+x_{2f}+x_{3f},
\]
\[
\sum_{s \in S} y_{ls} \le 1,\qquad g_{ls} \le U_s y_{ls},
\]
\[
\sum_{s \in S} y_{ls} = 0 \quad \forall l \in L_{\text{blocked}},\qquad
\sum_{s \in S} y_{l_1s} + \sum_{s \in S} y_{l_2s} \le 1 \quad \forall (l_1,l_2)\in \mathcal{C},
\]
\[
\mathrm{existing\_total}_z + \sum_{f \in F(z)}(x_{1f}+x_{2f}+x_{3f}) + \sum_{(l,s)\in L(z)} B_s y_{ls} \ge \mathrm{req\_total}_z,
\]
\[
\mathrm{existing\_under5}_z + \sum_{f \in F(z)}u_f + \sum_{(l,s)\in L(z)} g_{ls} \ge \mathrm{req\_under5}_z.
\]
These choices match the NYC planning context described in the assignment. Banded incumbent variables make the tighter \(20\%\) cap and increasing marginal expansion difficulty transparent, while candidate-site binary variables make the \(0.06\)-mile screening rule enforceable. Because existing facilities are fixed assets in the current planning horizon, existing-existing proximity issues are tracked as diagnostics rather than modeled as closures or relocations.

\begin{resultbox}
\textbf{Implemented Part 2 model.}
The realistic optimization stage solves a citywide MILP with \(201{,}980\) variables and \(155{,}819\) constraints. The model enforces the coded site-screening and service constraints, but the expansion-cost objective is an additive increasing-cost approximation rather than the assignment's literal piecewise-total formula. Existing-existing distance conflicts are recorded diagnostically, not enforced.
\end{resultbox}
...
The benchmark optimization stage solves the Part 1 model to proven optimality in \(3.73\) seconds. The implemented benchmark objective is \(\$126.67\) million, split across expansion, new construction, and under-\(5\) equipment.
...
\begin{tabular}{lr}
\toprule
\textbf{Part 1 metric} & \textbf{Value} \\
\midrule
Total objective (\$) & 126,668,941 \\
Expansion cost (\$) & 54,716,841 \\
New-build cost (\$) & 44,180,000 \\
Under-\(5\) equipment cost (\$) & 27,772,100 \\
Added expansion slots & 206,364 \\
Added under-\(5\) expansion slots & 202,938 \\
New facilities built & 386 \\
Added new-build slots & 152,900 \\
Added under-\(5\) new-build slots & 74,783 \\
Zip codes meeting all coded requirements after solve & 311 \\
\bottomrule
\end{tabular}
...
The realistic optimization stage solves the Part 2 model in \(18.21\) seconds with a final MIP gap below \(0.1\%\). The objective rises to \(\$183.96\) million, and the intervention mix shifts sharply toward new construction.
...
\begin{tabular}{lr}
\toprule
\textbf{Part 2 metric} & \textbf{Value} \\
\midrule
Total objective (\$) & 183,963,207 \\
Expansion cost (\$) & 9,381,107 \\
New-build cost (\$) & 146,810,000 \\
Under-\(5\) equipment cost (\$) & 27,772,100 \\
Expanded facilities & 1,126 \\
Added expansion slots & 25,108 \\
Added under-\(5\) expansion slots & 25,067 \\
New facilities built & 1,284 \\
Added new-build slots & 507,700 \\
Added under-\(5\) new-build slots & 252,654 \\
Zip codes with zero under-\(5\) slack & 311 \\
Zip codes meeting all coded requirements after solve & 311 \\
\bottomrule
\end{tabular}
\caption{Implemented Part 2 realistic results}
\label{tab:part2-results}
\end{table}

\begin{resultbox}
\textbf{Validation checks.}
The exported validation table from the realistic optimization stage reports \(311/311\) zip codes meeting both service requirements, \(0\) blocked candidate sites selected, \(0\) candidate-conflict violations, \(0\) facility cap violations, and \(0\) new builds in zero-child zip codes.
\end{resultbox}
...
The highest-intervention zip codes remain concentrated in a relatively small set of areas, including \(11219\), \(11230\), \(11368\), \(11206\), \(11223\), and \(11204\). In those locations, limited incumbent expansion room and strong under-\(5\) demand force new construction rather than marginal adjustments to existing facilities.
...
The key policy insight is not just that the realistic model is more expensive. It is that the intervention mechanism changes. Once expansion becomes tightly capped and candidate-site spacing is enforced, the city must fund many more large new facilities. Across both parts, the under-\(5\) requirement is the dominant planning driver.
